...
export default function Home() {
  ...
  const handleDownload = async () => {
    ...
    let currentSubgraphIndex = 0;

    try {
      // Process each subgraph sequentially
      for (const subgraph of subgraphs) {
        if (!subgraph.id.trim()) continue; // Skip empty subgraph IDs

        currentSubgraphIndex++;
        appendLog(`Processing subgraph ${currentSubgraphIndex}/${subgraphs.length}: ${subgraph.id}`);
